...
import { IMediaPost, IPost, IPostWithUser } from '../interfaces/others.model';
...
<div *ngIf="mergedPost$ | async" class="h-screen mt-8 overflow-y-scroll bg-gray-100 flex flex-col items-center">
   <div class="flex flex-col pt-12 w-full lg:w-[70%] md:w-[70%]">
      <ng-container *ngFor="let postWithUser of mergedPost$ | async">
            <div *ngIf="isMediaPost(postWithUser.post) && postWithUser.post.title !== undefined; else withoutImageTemplate" class="bg-white shadow rounded-lg my-4">
               <ng-container *ngIf="postWithUser.user as data">
                  <div class="relative">
                     <button (click)="toggleDropdown(postWithUser.post.id)" class="absolute top-4 right-6 p-2 text-sm font-medium text-center text-gray-900 bg-white rounded-lg hover:bg-gray-100 focus:ring-4 focus:outline-none dark:text-white focus:ring-gray-50 dark:bg-gray-900 dark:hover:bg-gray-800 dark:focus:ring-gray-600" type="button">
                        <svg class="w-4 h-4 text-gray-500 dark:text-gray-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 4 15">
                           <path d="M3.5 1.5a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0Zm0 6.041a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0Zm0 5.959a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0Z"/>
                        </svg>
                     </button>
                     <!-- Dropdown content -->
                     <div *ngIf="isDropdownOpen(postWithUser.post.id)" class="dropdown-content flex justify-evenly pl-2 pt-4 pb-2">
                        <!-- Add your dropdown buttons here -->
                        <button (click)="viewPost(postWithUser.id)" class="w-full text-center py-2 px-4 text-left hover:bg-gray-100"><svg fill="#8c8c8c" width="30px" height="30px" viewBox="-3.5 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg" stroke="#8c8c8c"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <title>view</title> <path d="M12.406 13.844c1.188 0 2.156 0.969 2.156 2.156s-0.969 2.125-2.156 2.125-2.125-0.938-2.125-2.125 0.938-2.156 2.125-2.156zM12.406 8.531c7.063 0 12.156 6.625 12.156 6.625 0.344 0.438 0.344 1.219 0 1.656 0 0-5.094 6.625-12.156 6.625s-12.156-6.625-12.156-6.625c-0.344-0.438-0.344-1.219 0-1.656 0 0 5.094-6.625 12.156-6.625zM12.406 21.344c2.938 0 5.344-2.406 5.344-5.344s-2.406-5.344-5.344-5.344-5.344 2.406-5.344 5.344 2.406 5.344 5.344 5.344z"></path> </g></svg></button>
                        <button (click)="editPost(postWithUser.id)" class="w-full text-center py-2 px-4 text-left hover:bg-gray-100"><svg width="30px" height="30px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="#7a7a7a"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"><path fill-rule="evenodd" clip-rule="evenodd" d="m3.99 16.854-1.314 3.504a.75.75 0 0 0 .966.965l3.503-1.314a3 3 0 0 0 1.068-.687L18.36 9.175s-.354-1.061-1.414-2.122c-1.06-1.06-2.122-1.414-2.122-1.414L4.677 15.786a3 3 0 0 0-.687 1.068zm12.249-12.63 1.383-1.383c.248-.248.579-.406.925-.348.487.08 1.232.322 1.934 1.025.703.703.945 1.447 1.025 1.934.058.346-.1.677-.348.925L19.774 7.76s-.353-1.06-1.414-2.12c-1.06-1.062-2.121-1.415-2.121-1.415z" fill="#575757"></path></g></svg></button>
                        <button (click)="deletePost(postWithUser.post)" class="w-full text-center py-2 px-4 text-left hover:bg-gray-100"><svg width="30px" height="30px" viewBox="0 0 1024 1024" class="icon" version="1.1" xmlns="http://www.w3.org/2000/svg" fill="#000000"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"><path d="M154 260h568v700H154z" fill="#b92f27"></path><path d="M624.428 261.076v485.956c0 57.379-46.737 103.894-104.391 103.894h-362.56v107.246h566.815V261.076h-99.864z" fill="#696969"></path><path d="M320.5 870.07c-8.218 0-14.5-6.664-14.5-14.883V438.474c0-8.218 6.282-14.883 14.5-14.883s14.5 6.664 14.5 14.883v416.713c0 8.219-6.282 14.883-14.5 14.883zM543.5 870.07c-8.218 0-14.5-6.664-14.5-14.883V438.474c0-8.218 6.282-14.883 14.5-14.883s14.5 6.664 14.5 14.883v416.713c0 8.219-6.282 14.883-14.5 14.883z" fill="#152B3C"></path><path d="M721.185 345.717v-84.641H164.437z" fill="#696969"></path><path d="M633.596 235.166l-228.054-71.773 31.55-99.3 228.055 71.773z" fill="#b92f27"></path><path d="M847.401 324.783c-2.223 0-4.475-0.333-6.706-1.034L185.038 117.401c-11.765-3.703-18.298-16.239-14.592-27.996 3.706-11.766 16.241-18.288 27.993-14.595l655.656 206.346c11.766 3.703 18.298 16.239 14.592 27.996-2.995 9.531-11.795 15.631-21.286 15.631z" fill="#b92f27"></path></g></svg></button>
                     </div>
                     <div class="flex flex-row px-2 py-3 mx-3">
                        <div class="w-auto h-auto rounded-full border-2 border-green-500">
                           <img class="w-12 h-12 object-cover rounded-full shadow cursor-pointer" alt="User avatar" [src]="data?.image">
                        </div>
                        <div class="flex flex-col mb-2 ml-4 mt-1">
                           <div class="text-gray-600 text-sm font-semibold">{{data.username}}</div>
                           <div class="flex w-full mt-1">
                              <div class="text-blue-700 font-base text-xs mr-1 cursor-pointer">
                                 UX Design
                              </div>
                              <div class="text-gray-400 font-thin text-xs">
                                 • {{postWithUser.post.timestamp.toDate() | date:"MMM dd, yyyy 'at' hh:mm a"}}
                              </div>
                           </div>
                        </div>
                     </div>
                     ...
                     <div class="text-gray-400 font-medium text-sm mb-7 mt-6 mx-3 px-2">
                        <img class="rounded w-full" [src]="postWithUser.post.images[0]">
                     </div>
                     <div class="text-gray-600 font-semibold  mb-2 mx-3 px-2" [innerText]="postWithUser.post.title"></div>
                     <div class="text-gray-500 text-sm mb-6 mx-3 px-2" [innerText]="postWithUser.post.content"></div>
                     ...
                     <div class="flex w-full border-t border-gray-100">
                        ...
                        <div class="mt-3 mx-5 w-full flex justify-end text-xs">
                           <div class="flex text-gray-700  rounded-md mb-2 mr-4 items-center">
                              Likes: 
                              <div class="ml-1 text-gray-400 text-ms">{{postWithUser.post.likes}}</div>
                           </div>
                        </div>
                     </div>
                     <div class="relative flex items-center self-center w-full max-w-xl p-4 overflow-hidden text-gray-600 focus-within:text-gray-400">
                        <img class="w-10 h-10 object-cover rounded-full shadow mr-2 cursor-pointer" alt="User avatar" [src]="data.image">
            ...
            <ng-template #withoutImageTemplate let-postIndex="index">
               <div class="relative rounded-xl border p-5 shadow-md w-full bg-white my-2">
                  ...
                  <!-- Dropdown content -->
                  <div *ngIf="isDropdownOpen(postWithUser.post.id)" class="dropdown-content flex justify-evenly mt-2 pb-2">
                     <!-- Add your dropdown buttons here -->
                     <button (click)="viewPost(postWithUser.post.id)" class="w-full text-center py-2 px-4 text-left hover:bg-gray-100"><svg fill="#8c8c8c" width="30px" height="30px" viewBox="-3.5 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg" stroke="#8c8c8c"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <title>view</title> <path d="M12.406 13.844c1.188 0 2.156 0.969 2.156 2.156s-0.969 2.125-2.156 2.125-2.125-0.938-2.125-2.125 0.938-2.156 2.125-2.156zM12.406 8.531c7.063 0 12.156 6.625 12.156 6.625 0.344 0.438 0.344 1.219 0 1.656 0 0-5.094 6.625-12.156 6.625s-12.156-6.625-12.156-6.625c-0.344-0.438-0.344-1.219 0-1.656 0 0 5.094-6.625 12.156-6.625zM12.406 21.344c2.938 0 5.344-2.406 5.344-5.344s-2.406-5.344-5.344-5.344-5.344 2.406-5.344 5.344 2.406 5.344 5.344 5.344z"></path> </g></svg></button>
                     <button (click)="editPost(postWithUser.post.id)" class="w-full text-center py-2 px-4 text-left hover:bg-gray-100"><svg width="30px" height="30px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="#7a7a7a"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"><path fill-rule="evenodd" clip-rule="evenodd" d="m3.99 16.854-1.314 3.504a.75.75 0 0 0 .966.965l3.503-1.314a3 3 0 0 0 1.068-.687L18.36 9.175s-.354-1.061-1.414-2.122c-1.06-1.06-2.122-1.414-2.122-1.414L4.677 15.786a3 3 0 0 0-.687 1.068zm12.249-12.63 1.383-1.383c.248-.248.579-.406.925-.348.487.08 1.232.322 1.934 1.025.703.703.945 1.447 1.025 1.934.058.346-.1.677-.348.925L19.774 7.76s-.353-1.06-1.414-2.12c-1.06-1.062-2.121-1.415-2.121-1.415z" fill="#575757"></path></g></svg></button>
                     <button (click)="deletePost(postWithUser.post)" class="w-full text-center py-2 px-4 text-left hover:bg-gray-100"><svg width="30px" height="30px" viewBox="0 0 1024 1024" class="icon" version="1.1" xmlns="http://www.w3.org/2000/svg" fill="#000000"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"><path d="M154 260h568v700H154z" fill="#b92f27"></path><path d="M624.428 261.076v485.956c0 57.379-46.737 103.894-104.391 103.894h-362.56v107.246h566.815V261.076h-99.864z" fill="#696969"></path><path d="M320.5 870.07c-8.218 0-14.5-6.664-14.5-14.883V438.474c0-8.218 6.282-14.883 14.5-14.883s14.5 6.664 14.5 14.883v416.713c0 8.219-6.282 14.883-14.5 14.883zM543.5 870.07c-8.218 0-14.5-6.664-14.5-14.883V438.474c0-8.218 6.282-14.883 14.5-14.883s14.5 6.664 14.5 14.883v416.713c0 8.219-6.282 14.883-14.5 14.883z" fill="#152B3C"></path><path d="M721.185 345.717v-84.641H164.437z" fill="#696969"></path><path d="M633.596 235.166l-228.054-71.773 31.55-99.3 228.055 71.773z" fill="#b92f27"></path><path d="M847.401 324.783c-2.223 0-4.475-0.333-6.706-1.034L185.038 117.401c-11.765-3.703-18.298-16.239-14.592-27.996 3.706-11.766 16.241-18.288 27.993-14.595l655.656 206.346c11.766 3.703 18.298 16.239 14.592 27.996-2.995 9.531-11.795 15.631-21.286 15.631z" fill="#b92f27"></path></g></svg></button>
                  </div>
                  <!-- Existing content for posts without images -->
                  <div class="flex w-full items-center justify-between border-b">
                     <div *ngIf="postWithUser.user as data" class="flex flex-col items-start">
                        <div class="flex items-center">
                           <img *ngIf="data?.image" [src]="data.image" alt="User Image" class="h-8 w-8 rounded-full">
                           <div class="text-lg font-bold text-slate-700 px-4" [innerText]="data.username"></div>
                        </div>
                        <div class="ml-12 text-xs text-neutral-500">{{ postWithUser.post.timestamp.toDate() | date:"MMM dd, yyyy 'at' hh:mm a" }}</div>
                     </div>
                  </div>
                  <div class="mt-4 mb-6">
                     <div class="mb-3 text-xl font-bold" [innerText]="postWithUser.post.title"></div>
                     <div class="text-sm text-neutral-600" [innerText]="postWithUser.post.content"></div>
                  </div>
                  <div>
                     <div class="flex items-center justify-between text-slate-500">
                        <div class="flex space-x-4 md:space-x-8">
                           ...
                           <div class="flex cursor-pointer items-center transition hover:text-slate-600">
                              ...
                              <span>{{postWithUser.post.likes}}</span>
                           </div>
                        </div>
                     </div>
                  </div>
 ...
 export class PostsComponent implements OnInit {
   ...
   mergedPost$ = new BehaviorSubject<(IPostWithUser)[]>([]);
   ...
   ngOnInit(): void {
      const posts = this.postService.getAllPostsWithUsers();
